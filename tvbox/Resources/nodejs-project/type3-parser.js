...
async function fetchSpiderFromJar(spiderUrl) {
    ...
    if (buffer.length >= 4 && buffer.readUInt32LE(0) === 0x04034b50) {
        ...
        if (content.startsWith('<?xml') || content.includes('<Error>') || content.includes('resource not found')) {
            throw new Error(`下载到错误页面: ${safePreview(content, 200)}，可能是Jar源服务器拒绝了访问`);
        }
...
// 增强版下载：支持重定向（301/302）和 gzip 解压，用安卓的User-Agent骗过服务器
...
function getDefaultHeaders() {
    // 改成安卓的User-Agent！骗过Jar源的服务器，让它以为我们是安卓的TVBox！
    return { 
        'User-Agent': 'Mozilla/5.0 (Linux; Android 10; SM-G973N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.120 Mobile Safari/537.36',
