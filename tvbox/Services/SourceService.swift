...
class SourceService {
    ...
    func getSort(sourceBean: SourceBean) async throws -> (sorts: [MovieSort.SortData], homeVideos: [Movie.Video]) {
        ...
        if sourceBean.type == 3 {
            return try await getSpiderSort(sourceBean: sourceBean)
        }
    ...
    private func parseSort(_ jsonStr: String, sourceBean: SourceBean) throws -> (sorts: [MovieSort.SortData], homeVideos: [Movie.Video]) {
        ...
        if sourceBean.type == 0 {
            ...
            if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                var filtersMap: [String: [MovieSort.SortFilter]] = [:]
                if let filters = json["filters"] as? [String: [[String: Any]]] {
                    for (typeId, filterList) in filters {
                        var parsedFilters: [MovieSort.SortFilter] = []
                        for filter in filterList {
                            let key = filter["key"] as? String ?? ""
                            let name = filter["name"] as? String ?? key
                            var values: [MovieSort.SortFilter.SortFilterValue] = []
                            if let valueList = filter["value"] as? [[String: Any]] {
                                for v in valueList {
                                    let n = v["n"] as? String ?? ""
                                    let vv = v["v"] as? String ?? ""
                                    if !n.isEmpty || !vv.isEmpty {
                                        values.append(MovieSort.SortFilter.SortFilterValue(n: n, v: vv))
                                    }
                                }
                            }
                            if !key.isEmpty {
                                parsedFilters.append(MovieSort.SortFilter(key: key, name: name, values: values))
                            }
                        }
                ...
                if let classList = json["class"] as? [[String: Any]] {
                    for cls in classList {
                        ...
                        var sortData = MovieSort.SortData(id: id, name: name)
                        sortData.filters = filtersMap[id] ?? []
                        sorts.append(sortData)
                    }
                }
    ...
    func getList(sourceBean: SourceBean, sortData: MovieSort.SortData, page: Int = 1, filters: [String: String]? = nil) async throws -> [Movie.Video] {
        ...
        if sourceBean.type == 3 {
            return try await getSpiderList(sourceBean: sourceBean, sortData: sortData, page: page, filters: filters)
        }
    ...
    func getDetail(sourceBean: SourceBean, vodId: String) async throws -> VodInfo? {
        ...
        guard !api.isEmpty else { throw SourceError.emptyApi }
        if sourceBean.type == 3 {
            return try await getSpiderDetail(sourceBean: sourceBean, vodId: vodId)
        }
        guard sourceBean.isSupportedInSwift else { throw SourceError.unsupportedType(sourceBean.typeDescription) }
        guard sourceBean.isHttpApi else { throw SourceError.invalidApiUrl(api) }
    ...
    func search(sourceBean: SourceBean, keyword: String) async throws -> [Movie.Video] {
        ...
        guard !api.isEmpty else { throw SourceError.emptyApi }
        if sourceBean.type == 3 {
            return try await spiderSearch(sourceBean: sourceBean, keyword: keyword)
        }
        guard sourceBean.isSupportedInSwift else { throw SourceError.unsupportedType(sourceBean.typeDescription) }
        guard sourceBean.isHttpApi else { throw SourceError.invalidApiUrl(api) }
    ...
    func searchAll(keyword: String) async -> [Movie.Video] {
        ...
        return await withTaskGroup(of: [Movie.Video].self) { group in
            for source in sources {
                guard source.isSupportedInSwift else { continue }
    ...
    private func getSpiderSort(sourceBean: SourceBean) async throws -> (sorts: [MovieSort.SortData], homeVideos: [Movie.Video]) {
        ...
        var filtersMap: [String: [MovieSort.SortFilter]] = [:]
        if let filters = result["filters"] as? [String: [[String: Any]]] {
            for (typeId, filterList) in filters {
                var parsedFilters: [MovieSort.SortFilter] = []
                for filter in filterList {
                    let key = filter["key"] as? String ?? ""
    ...
    private func getSpiderDetail(sourceBean: SourceBean, vodId: String) async throws -> VodInfo? {
        ...
        guard let list = result["list"] as? [[String: Any]],
